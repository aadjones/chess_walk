...
def create_position_controls(position_ids, total_positions=None, all_position_ids=None):
    ...
    with col1:
        st.button(
            "← Previous", on_click=prev_position, key="prev_button", disabled=disable_prev, use_container_width=True
        )
    with col2:
        st.button("Next →", on_click=next_position, key="next_button", disabled=disable_next, use_container_width=True)
    ...
    selected_position_id = st.sidebar.number_input(
        label, min_value=min_pos, max_value=max_pos, value=current_pos_id, step=1, key="position_number_input"
    )
